#let MainPageSettings(doc) = {
  ...
  // PAGE SETTINGS (UoL PhD Thesis Guidelines)
  // Left >= 40mm, others >= 15mm
  ...
  // Footer with chapter name and page number
  set page(footer: context {
    if here().page() == 1 { return [] }

    let chapter-title = []

    // Find the current chapter heading
    let headings = query(heading.where(level: 1).before(here()))
    if headings.len() > 0 {
      let last = headings.last()
      chapter-title = last.body
    }
    ...
    return block(width: 100%)[
      #set text(size: 9pt)
      #align(right)[
        #emph(chapter-title) #h(0.25cm) #strong(page-num)
      ]
    ]
  })
